...
\paragraph{Proof} Consider the Cantor fractal:\\ \vbox{
...
\end{verbatim}
The 2-kernel of this sequence contains the words:\\ \vbox{
\begin{verbatim}
...
\paragraph{Proof} By induction:\\
Initial step:
$\sigma^2 = aba = \sigma^1 \sigma^0, \sigma^3 = abaab = \sigma^2 \sigma^1$.\\
Induction step: $\sigma^{n + 1} = \sigma^n \sigma^{n - 1}$.\\
We know that $\sigma^{n + 1} = \sigma(\sigma^n)$, so we can re-write this as:
$\sigma^{n + 1} = \sigma(\sigma^{n - 1} \sigma^{n - 2}$, because of our
hypothesis. This equals $\sigma(\sigma^{n - 1}) \sigma(\sigma^{n - 2}) = \sigma^n \sigma^{n - 1}$. \qed\\ \vbox{
\begin{verbatim}
  a|b|a|ab|aba|abaab ...
  0|1|1|11|111|11111
      0|00|000|00000
        01|001|00011
           010|00100
               01001
\end{verbatim}}
We can see that the Fibonacci expansions of the integers in $\sigma^n$ are all
of length $F_n$ and we can deduce from the definition of the Fibonacci
expansion that each string $...0 \rightarrow ...00, ...01$, and
$...1 \rightarrow ...10$ in a 1-1 correspondence with the fixed point.\\
