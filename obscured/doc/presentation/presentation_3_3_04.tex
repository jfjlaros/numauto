...
%(in direct reading, or sometimes more in reverse reading, but we will discuss
%that later on)
...
%This is equivalent to reverse reading. This will be proved at the end of the
%next section.\\
...
$\mathcal{S} = \{a_1, ..., a_d\}$.
%Now make a bijection between $u$ and
%$\mathcal{S}$.
...
%\Up to now we have only looked at automata that had so-called regular (or
%\representable) input languages (moreover, our input language has always been
%\$\Delta^*$), but in general an automaton will only accept a subset of all
%words in an alphabet.\\
%\There is a limit to what a finite automaton can accept, we call this class of
